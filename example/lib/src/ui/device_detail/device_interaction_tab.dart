      ...
      if (Platform.isAndroid) {
        for (var element in result) {
          for (var inner in element.characteristics) {
            if (inner.isReadable && !isRead) {
              isRead = true;
              discoveredServices.add(element);
              continue;
            }
            if (inner.characteristicId == BLE_WRITE_UUID &&
                inner.isWritableWithResponse) {
              discoveredServices.add(element);
              continue;
            }
            if (inner.characteristicId == CLIENT_CHARACTERISTIC_CONFIG &&
                inner.isNotifiable) {
              discoveredServices.add(element);
              continue;
            }
        ...
        for (var element in result) {
          // 处理当前平台
          if (element.serviceId == IOS_BLE_PARENT_UUID) {
            discoveredServices = [element];
          }
        }
      }
    });
